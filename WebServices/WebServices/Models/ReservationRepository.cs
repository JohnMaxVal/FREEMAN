...
namespace WebServices.Models
{
    public class ReservationRepository
    {
        ...
        {
            new Reservation {
            ReservationId = 1, ClientName = "Eugene", Location = "Board Room"},
            new Reservation {
        ...
        {
            Reservation item = data.Where(i => i.ReservationId == id).FirstOrDefault();
            if(item !=null)
            {
                data.Remove(item);
            }
        ...
        {
            Reservation storedItem = data.Find(i => i.ReservationId == item.ReservationId);
